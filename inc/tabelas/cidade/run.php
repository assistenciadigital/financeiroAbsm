    ...
    <div class="col-sm-12">
        <h1 class="display-3">Aguarde, Processando...
              <?php
                    ...
                    $sql = $mysqli->query("SELECT id, uf, descricao FROM {$tabela} WHERE uf='".$_POST['uf']."' and descricao='".$_POST['descricao']."'"); // CONSULTA DUPLICIDADE
                
                    if (!$sql->num_rows){ //VALIDA DUPLICIDADE
                        if (isset($_GET['id']) and $_GET['acao'] == 'edicao'){ // SE FOR EDICAO
							if (($menu >= 1) AND ($menu <= 3)){ // NIVEL EDICAO
							
								$sql = "UPDATE {$tabela} SET uf='".$_POST['uf']."', descricao = '".trim(addslashes(htmlentities(ucfirst($_POST['descricao']))))."', data = '".date('Y-m-d H:i:s')."', usuario_login = '".$_SESSION['usuarioLogin']."'  WHERE id = '".$_GET['id']."'";
							...
							if (($menu >= 1) AND ($menu <= 4)){ // NIVEL INCLUSAO
                                $sql = "INSERT INTO {$tabela} (descricao, uf, data, usuario_login) VALUES ('".trim(addslashes(htmlentities(ucfirst($_POST['descricao']))))."', '".$_POST['uf']."', '".date('Y-m-d H:i:s')."', '".$_SESSION['usuarioLogin']."')";
