...
include("../../contadorVisitas.php");
	...
	//CONECTA COM O MYSQL
	$select = mysql_query("select *, date_format(`data`,'%d/%m/%Y %H:%i') as `data_formatada` from cidade where uf like '%".$_GET['uf']."%' and descricao like '%".$_GET['consulta']."%' order by uf, descricao");
...
class PDF extends FPDF
{
   ...
   function Header() {
	  ...
	  $this->Cell(0,4,'RELACAO DE CIDADE',0,1,'C');
...
if (mysql_num_rows ($select)) {
  while ($row = mysql_fetch_assoc($select)) {
		...
		$pdf->Cell(111, 4, $row['descricao'].' - '.$row['uf'], 1, 0, 'L');
...
$pdf-> Cell(0,4, number_format(($qtde_pesquisado), 0, ',', '.').' de '.number_format(($qtde_registro), 0, ',', '.')." Registro(s) para: UF = ".$_GET['uf'].' e Cidade = '.$_GET['consulta'].' | Ordenado por: UF, Descricao',1,0,'C');
