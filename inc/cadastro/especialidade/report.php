...
include("../../contadorVisitas.php");
	...
	//CONECTA COM O MYSQL
	$select = mysql_query("select *, date_format(`data`,'%d/%m/%Y %H:%i') as `data_formatada` from especialidade where tipo like '%".$_GET['tipo']."%' and descricao like '%".$_GET['consulta']."%' order by tipo, descricao");
...
class PDF extends FPDF
{
   ...
   function Header() {
	  ...
	  $this->Cell(0,4,'RELACAO DE ESPECIALIDADE MEDICA / ODONTOLOGICA',0,1,'C');
	  ...
	  //MONTA O CABEÇALHO DA TABELA
		...
		$this->Cell(30, 4, "TIPO", 1, 0, 'C');
		$this->Cell(81, 4, "DESCRICAO", 1, 0, 'J');
...
if (mysql_num_rows ($select)) {
  while ($row = mysql_fetch_assoc($select)) {
		...
		$pdf->Cell(30, 4, $row['tipo'], 1, 0, 'L');
		$pdf->Cell(81, 4, $row['descricao'], 1, 0, 'J');
...
$pdf-> Cell(0,4, number_format(($qtde_pesquisado), 0, ',', '.').' de '.number_format(($qtde_registro), 0, ',', '.')." Registro(s) para: Tipo = ".$_GET['tipo'].' e Especialidade = '.$_GET['consulta'].' | Ordenado por: Tipo, Descricao',1,0,'C');
