...
class PDF extends FPDF
{
   ...
   function Header() {
	  ...
	  $this->Cell(0,4,'RELACAO DE MENU',0,1,'C');
	  ...
	  //MONTA O CABEÇALHO DA TABELA
		...
		$this->Cell(35, 4, "MENU", 1, 0, 'L');
		$this->Cell(38, 4, "SUB-MENU", 1, 0, 'C');
		$this->Cell(50, 4, "DESCRICAO", 1, 0, 'C');
		$this->Cell(60, 4, "LINK", 1, 0, 'C');
		$this->Cell(30, 4, "ARQUIVO", 1, 0, 'C');
...
if (mysql_num_rows ($select)) {
  while ($row = mysql_fetch_assoc($select)) {
		$pdf->Cell(10, 4, $row['id'], 1, 0, 'C');
		$pdf->Cell(35, 4, $row['menu'], 1, 0, 'L');
		$pdf->Cell(38, 4, $row['submenu'], 1, 0, 'L');
		$pdf->Cell(50, 4, $row['descricao'], 1, 0, 'L');
		$pdf->Cell(60, 4, $row['link'], 1, 0, 'L');
		$pdf->Cell(30, 4, $row['arquivo'], 1, 0, 'L');
		$pdf->Cell(29, 4, $row['data_formatada'], 1, 0, 'L');
		$pdf->Cell(25, 4, strtoupper($row['usuario_login']), 1, 1, 'L');
  }
}
