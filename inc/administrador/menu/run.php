    ...
    <div class="col-sm-12">
        <h1 class="display-3">Aguarde, Processando...
              <?php
                    ...
                    $sql = $mysqli->query("SELECT id FROM {$tabela} WHERE menu='".trim(addslashes(htmlentities(ucfirst($_POST['menu']))))."' and submenu='".trim(addslashes(htmlentities(ucfirst($_POST['submenu']))))."' and descricao='".trim(addslashes(htmlentities(ucfirst($_POST['descricao']))))."' and arquivo='".trim(addslashes(htmlentities(strtolower($_POST['arquivo']))))."' and link='".trim(addslashes(htmlentities(strtolower($_POST['link']))))."'"); // CONSULTA DUPLICIDADE
                
                    if (!$sql->num_rows){ //VALIDA DUPLICIDADE
                        if (isset($_GET['id']) and $_GET['acao'] == 'edicao'){ // SE FOR EDICAO
						if (($menu >= 1) AND ($menu <= 3)){ // NIVEL EDICAO
                            $sql = "UPDATE {$tabela} SET menu = '".trim(addslashes(htmlentities(ucfirst($_POST['menu']))))."', submenu = '".trim(addslashes(htmlentities(ucfirst($_POST['submenu']))))."', descricao = '".trim(addslashes(htmlentities(ucfirst($_POST['descricao']))))."', link = '".trim(addslashes(htmlentities(strtolower($_POST['link']))))."', arquivo = '".trim(addslashes(htmlentities(strtolower($_POST['arquivo']))))."',  data = '".date('Y-m-d H:i:s')."', usuario_login = '".$_SESSION['usuarioLogin']."'  WHERE id = '".$_GET['id']."'";
	                        $query = $mysqli->query($sql);
                        ...
                        if ($query) $resultado = "Registro: ".$_GET['id'].", Alterado com Sucesso!"; else $resultado = "Erro ao Alterar o Registro: ".$_GET['id']."."; echo "<script language='javascript' TYPE='text/javascript'>alert ('$resultado')</script>";
							...
							if (($menu >= 1) AND ($menu <= 4)){ // NIVEL INCLUSAO
                                $sql = "INSERT INTO {$tabela} (menu, submenu, descricao, arquivo, link, data, usuario_login) VALUES ('".trim(addslashes(htmlentities(ucfirst($_POST['menu']))))."', '".trim(addslashes(htmlentities(ucfirst($_POST['submenu']))))."', '".trim(addslashes(htmlentities(ucfirst($_POST['descricao']))))."', '".trim(addslashes(htmlentities(strtolower($_POST['arquivo']))))."', '".trim(addslashes(htmlentities(strtolower($_POST['link']))))."', '".date('Y-m-d H:i:s')."', '".$_SESSION['usuarioLogin']."')";
