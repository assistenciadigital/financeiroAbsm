...
//CONECTA COM O MYSQL
$select = mysql_query("select *, date_format(`hora`,'%d/%m/%Y %H:%i') as `data_formatada` from {$tabela} where {$pesquisarpor} like '%".$_GET['consulta']."%' order by {$orderby}");
...
class PDF extends FPDF
{
   ...
   function Header() {
	  ...
	  $this->Cell(0,4,'RELACAO DE LOGs',0,1,'C');
	  ...
	  //MONTA O CABEÇALHO DA TABELA
		...
		$this->Cell(29, 4, "DATA / HORA", 1, 0, 'C');
        $this->Cell(27, 4, "IP", 1, 0, 'C');
        $this->Cell(30, 4, "USUARIO", 1, 0, 'C');
		$this->Cell(84, 4, "DESCRICAO", 1, 1, 'C');
   }
...
if (mysql_num_rows ($select)) {
  while ($row = mysql_fetch_assoc($select)) {
		$pdf->Cell(20, 4, $row['id'], 'LTR', 'R', 0);
		$pdf->Cell(29, 4, $row['data_formatada'], 'LTR', 'R', 0);
        $pdf->Cell(27, 4, $row['ip'], 'LTR', 'R', 0);
        $pdf->Cell(30, 4, $row['usuario'], 'LTR', 'R', 0);
        $pdf->MultiCell(84, 4, $row['mensagem'], 'LTR', 'L', 0);
		//$pdf->MultiCell(84, 4, strtoupper($row['mensagem']), 1, 1, 'L');
  }
}
