...
class PDF extends FPDF
{
   ...
   function Header() {
	  ...
	  $this->Ln(8);
	  $this->SetFont('Times','B',10);
	  $this->Cell(0,5, 'DECLARACAO ANUAL DE QUITACAO DE DEBITOS', 0, 0, 'C');
	  $this->Ln(8);
	  $this->SetFont('Times','',10);
	  $this->MultiCell(0,5,'De acordo com a Lei 12.007/2009, serve a presente como documento de quitacao das parcelas de seu contrato referente ao ano de '.$_GET['porano'].'.',0,'J',0);
	  $this->MultiCell(0,5,'O valor total de pagamento de R$ '.number_format($total['valortotal'], 2, ',', '.').' ('.strtoupper (trim(valor_extenso(number_format($total['valortotal'], 2, ',', '.')))).').',0,'J',0);
	  $this->Ln();
	  $this->SetFont('Times','B',10);
	  ...
	  //MONTA O CABEÇALHO DA TABELA
		...
		$this->Cell( 22, 5, "REFERENCIA", 1, 0, 'C');
		$this->Cell( 23, 5, "VENCIMENTO", 1, 0, 'C');
		$this->Cell( 145, 5, "SITUACAO", 1, 1, 'C');
...
if (mysql_num_rows ($select)) {
  while ($row = mysql_fetch_assoc($select)) {
		...
		$pdf->Cell(22, 6, mes_abreviado($row['mes']), 1, 0, 'C');
		$pdf->Cell(23, 6, number_format($row['valor_recebimento'], 2, ',', '.'), 1, 0, 'R');
		$pdf->Cell(145, 6, $row['evento'], 1, 1, 'L');		
		  }
}

$pdf-> Cell(45, 5, 'VALOR TOTAL',1,0,'C');
$pdf-> Cell(0, 5, 'R$ '.number_format(($somavalor).' ('.ucwords(trim(valor_extenso(number_format($somavalor, 2, ',', '.')))).'#', 2, ',', '.'),1,1,'L');
$pdf-> Ln(5);
$pdf->SetFont('Times','B',10);
$pdf-> Cell(0,6, 'NAO DEDUTIVEL PARA FINS DE IMPOSTO DE RENDA DE PESSOA FISICA - PF.',0,0,'C');
$pdf-> Ln(10);
$pdf->SetFont('Times','',10);
