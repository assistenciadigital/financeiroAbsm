...
//  Pesquisar por
    $pesquisar_por = (isset($_GET['pesquisarpor'])) ? $_GET['pesquisarpor'] : 'cliente';

//  Ordenar por
    $ordenar_por = (isset($_GET['pesquisarpor'])) ? $_GET['pesquisarpor'] : 'cliente';

//  Parametro Consulta
    //if ($pesquisar_por == 'cliente') $parametro = ' = '.$_GET['consulta']; else $parametro = " like '%".$_GET['consulta']."%'";
...
?>
<form method="get" name="frm" id="frm" action="titular.php">
<header>
	<div class="row col-xs-12 col-sm-12 col-md-12 col-lg-12">
		<div class="col-sm-12"> 
            <h2>Mensalidade - Titular</h2>
        </div>
    </div>
</header>        
    ...
    <div class="row">
        ...
        <div class="col-md-6 text-right h5">
            ...
            <select class="selectpicker" name="pesquisarpor" id="pesquisarpor" onchange="atualiza()">
                  <option selected value="cliente"<?php  if($_GET['pesquisarpor'] == 'cliente')  echo "selected"?>>Id</option>
                  <option value="nome"<?php  if($_GET['pesquisarpor'] == 'nome')  echo "selected"?>>Nome</option>
                  <option value="cpf"<?php  if($_GET['pesquisarpor'] == 'cpf')  echo "selected"?>>CPF</option>
            </select>
             Registros/Página:
...
<table class="table table-layout: fixed table-condensed h6">
<thead>
	<tr vertical-align: middle;>
		<th width="10%">ID / Status</th>
		<th width="50%">Nome</th>
    ...
    ?>
<tbody>
	<tr <?php if ($dados['status']=='Ativo') echo "bgcolor=success"; else if ($dados['status']=='Inativo' or $dados['status']=='Excluido') echo "bgcolor=danger"; else if ($dados['status']=='Bloqueado') echo "bgcolor=info"; else if ($dados['status']=='Recadastrar') echo "bgcolor=warning"; ?>> 
		<td><?php echo '<b>'.$dados['cliente'].'</b><br/>'.$dados['status']; ?></td>
		<td><?php echo '<b>'.$dados['nome'].'</b><br/>Mae: '.$dados['mae']; ?></td>
        <td class="text-center"><?php echo $dados['cpf'].'<br/>'.date_data($dados['datanascimento']); ?></td>
        <td><?php echo datetime_datatempo($dados['data']).'<br/>'.$dados['usuario']; ?></td>
		<td class="actions text-right">
			<!--<a href="<?php if (($menu >= 1) AND ($menu <= 5)) echo 'view.php?id='.$dados['cliente']; ?>" class="btn btn-sm btn-success"><i class="glyphicon glyphicon-eye-open"></i></a>-->
			<a href="<?php if (($menu >= 1) AND ($menu <= 3)) echo 'lista.php?titular='.$dados['cliente']; ?>" class="btn btn-sm btn-warning"><i class="glyphicon glyphicon-usd"></i>Mensalidade</a>
			<!--<a href="<?php if (($menu >= 1) AND ($menu <= 2)) echo 'run.php?acao=exclusao&id='.$dados['cliente']; ?>" class="btn btn-sm btn-danger"><i class="glyphicon glyphicon-trash"></i></a>-->
		</td>
	</tr>
<?php } ?>
</tbody>
        ...
        <div class="col-sm-4">
            <div class="alert alert-success text-center" role="alert">
                ...
                if ($total > 0) {
                    ...
                    $proximo = $pagina +1; 
                ?>
                <a href="titular.php?consulta=<?php echo $_GET['consulta']?>&pesquisarpor=<?php if ($_GET['pesquisarpor']) echo $_GET['pesquisarpor']; else echo $pesquisar_por;?>&pagina=1&porpagina=<?php if ($por_pagina) echo $por_pagina; else echo $_GET['porpagina']?>" rel="Registro Primeiro"><span class="glyphicon glyphicon-step-backward" aria-hidden="true"></span>Primeiro</a>

                <a href="titular.php?consulta=<?php echo $_GET['consulta']?>&pesquisarpor=<?php if ($_GET['pesquisarpor']) echo $_GET['pesquisarpor']; else echo $pesquisar_por;?>&pagina=<?php if ($anterior == 0){$anterior = 1;} echo $anterior?>&porpagina=<?php if ($por_pagina) echo $por_pagina; else echo $_GET['porpagina']?>" rel="Registro Anterior"><span class="glyphicon glyphicon-chevron-left" aria-hidden="true"></span>Anterior</a>

                <a href="titular.php?consulta=<?php echo $_GET['consulta']?>&pesquisarpor=<?php if ($_GET['pesquisarpor']) echo $_GET['pesquisarpor']; else echo $pesquisar_por;?>&pagina=<?php if ($proximo > $paginas){echo $paginas; }else{echo $proximo;}?>&porpagina=<?php if ($por_pagina) echo $por_pagina; else echo $_GET['porpagina']?>" rel="Registro Próximo">Próximo<span class="glyphicon glyphicon-chevron-right" aria-hidden="true"></span></a>

                <a href="titular.php?consulta=<?php echo $_GET['consulta']?>&pesquisarpor=<?php if ($_GET['pesquisarpor']) echo $_GET['pesquisarpor']; else echo $pesquisar_por; ?>&pagina=<?php echo $paginas?>&porpagina=<?php if ($por_pagina) echo $por_pagina; else echo $_GET['porpagina']?>" rel="Registro Último">Último<span class="glyphicon glyphicon-step-forward" aria-hidden="true"></span></a>
