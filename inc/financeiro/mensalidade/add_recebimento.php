...
<header>
  ...
  <form data-toggle="validator" action="run_recebimento.php?acao=inclusao&id=<?php echo $_GET['id'].'&titular='.$_GET['titular'];?>" method="post">
    <div class="row">
  	...
  	<div class="row">
	    <div class="form-group col-md-2">
  	     <label>Data Competencia</label>
  	  	  <input type="date" class="form-control" disabled id="competencia" name="competencia" value="<?php echo $dados['competencia']?>" placeholder="Competencia" required>
  	    </div> 	
			
	
	    <div class="form-group col-md-2">
  	     <label>Valor Mensalidade R$</label>
  	  	   <input requ type="text" name="valor" disabled id="valor" class="form-control" size="18" maxlength="13" value="<?php echo number_format($dados['valor'], 2, ',', '.')?>" onblur="javascript:formataValorDigitado(this);" onkeyup="javascript:formataValorDigitado(this);">
  	    </div> 	
		
	    <div class="form-group col-md-2">
  	     <label>Data Vencimento</label>
  	  	  <input type="date" class="form-control" disabled id="vencimento" name="vencimento" value="<?php echo $dados['vencimento']?>" placeholder="Vencimento" required>
  	    </div> 	
		
		<div class="form-group col-md-2">
		<label>Evento</label>
		    <select class="form-control" name="evento" disabled required>
	 ...
	 <div class="row">
	 	...
	 	<div class="form-group col-md-2">
  	     <label>Valor Recebido R$</label>
  	  	   <input requ type="text" name="valorrecebido" id="valorvalorrecebido" class="form-control" size="18" maxlength="13" value="<?php echo number_format($dados['valor'], 2, ',', '.')?>" onblur="javascript:formataValorDigitado(this);" onkeyup="javascript:formataValorDigitado(this);" autofocus>
  	    </div> 	
	...
	<tr vertical-align: middle;>
		<th width="10%">ID</th>
		<th width="20%" class="text-right">R$ Recebido</th>
		<th width="20%" class="text-center">Recebimento</th>
    ...
    $sql = ("select r.id as id, r.mensalidade as mensalidade, r.valor as valor, r.recebimento as recebimento, r.usuario_login as usuario_login, r.data as data from financeiro_recebimento as r where r.mensalidade = '".$_GET['id']."'");
	...
	<tr>
		...
		<td><?php echo datetime_datatempo($dados['data']);?></td>
		<td><?php echo $dados['usuario_login'];?></td>
		<td class="actions text-right">
			<a href="<?php if (($menu >= 1) AND ($menu <= 3)) echo 'edit_recebimento.php?id='.$dados['id'].'&mensalidade='.$dados['mensalidade'].'&titular='.$_GET['titular'];?>" class="btn btn-sm btn-warning"><i class="glyphicon glyphicon-pencil"></i></a>
			<a href="<?php if (($menu >= 1) AND ($menu <= 2)) echo 'run_recebimento.php?acao=exclusao&id='.$dados['id'].'&mensalidade='.$dados['mensalidade'].'&titular='.$_GET['titular'];?>" class="btn btn-sm btn-danger"><i class="glyphicon glyphicon-trash"></i></a>
		</td>
	</tr>
	<?php } ?>
	<table class="table table-hover table-layout: fixed table-condensed h6">
	...
	<tr vertical-align: middle;>
		...
		<th width="20%" class="text-right"><?php echo number_format($somarecebido, 2, ',', '.');?></th>
