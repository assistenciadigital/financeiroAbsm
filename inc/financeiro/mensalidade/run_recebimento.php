...
//  Caso algo tenha dado errado, exibe uma mensagem de erro
    ...
    $result = $mysqli->query("SELECT * FROM {$tabela} WHERE mensalidade = '".$_GET['id']."'");
              ...
              <?php
                    ...
                    if (isset($_GET['id']) and $_GET['acao'] == 'exclusao'){ // SE FOR EXCLUSAO
                        if (($menu >= 1) AND ($menu <= 2)){ // NIVEL EXCLUSAO
							...
							echo "<script language='javascript'> window.location.href = 'add_recebimento.php?id=".$_GET['mensalidade']."&titular=".$_GET['titular']."' </script>";
                            }else{
                    ...
                    if (!$sql->num_rows){ //VALIDA DUPLICIDADE
                        ...
                        $sql = "UPDATE {$tabela} SET titular = '".$_GET['titular']."', competencia = '".$_POST['competencia']."', valor = '".moeda($_POST['valor'])."', evento= '".$_POST['evento']."', vencimento = '".$_POST['vencimento']."', data = '".date('Y-m-d H:i:s')."', usuario_login = '".$_SESSION['usuarioLogin']."'  WHERE id = '".$_GET['id']."'";
						...
						if (($menu >= 1) AND ($menu <= 4)){ // NIVEL INCLUSAO
                                ...
                                $sql = "INSERT INTO {$tabela} (mensalidade, valor, recebimento, usuario_login, data) VALUES ('".$_POST['id']."', '".moeda($_POST['valorrecebido'])."', '".($_POST['recebimento'])."', '".$_SESSION['usuarioLogin']."', '".date('Y-m-d H:i:s')."')";
								...
								echo "<script language='javascript' TYPE='text/javascript'>alert ('$resultado')</script>";      
								echo "<script language='javascript'> window.location.href = 'add_recebimento.php?id=".$_GET['id']."&titular=".$_GET['titular']."' </script>";
								}else{
