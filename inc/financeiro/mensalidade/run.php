              ...
              <?php
                    ...
                    if (!$sql->num_rows){ //VALIDA DUPLICIDADE
						...
						if (($menu >= 1) AND ($menu <= 4)){ // NIVEL INCLUSAO
                                ...
                                $sql = "INSERT INTO {$tabela} (titular, competencia, valor, vencimento, evento, usuario_login, data) VALUES ('".$_GET['titular']."', '".($_POST['competencia'])."', '".moeda($_POST['valor'])."', '".($_POST['vencimento'])."', '".($_POST['evento'])."', '".$_SESSION['usuarioLogin']."', '".date('Y-m-d H:i:s')."')";
